...
describe ('Test cases for the home page',()=>{
    ...
    it('test create an expense', async ()=>{
        ...
        let localExpenses = await page.evaluate(() => (window.localStorage.getItem('puppeteer_here')));
    ...
    it('test searching items', async ()=>{
        ...
        await page.evaluate((userExpenses)=>{
            window.localStorage.setItem('puppeteer_here', JSON.stringify(userExpenses));
        },userExpenses);
        ...
        const data = await page.$$eval('table tr', trs => trs.map((tr) => {
            // return tr.innerText;
