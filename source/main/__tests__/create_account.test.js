...
describe("Create account test cases", () => {
  ...
  const userPassRepeat =
    "div.create_account_form input[name='repeat_password']";

  var alertMsg;
  // go to webpage
  beforeAll(async () => {
    await page.goto(
      "http://127.0.0.1:5500/source/main/login_page/login_page.html"
    );
    await page.goto(
      "http://127.0.0.1:5500/source/main/create_account/create_account.html"
    );
  });
  ...
  it("empty username", async () => {
    ...
    let localUsers = await page.evaluate(() =>
